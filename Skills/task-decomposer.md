...
2. 新产品或重大用户旅程需要产品基线但尚未确认时，只拆调查、PRD、原型和
   可逆技术探针；不生成冻结 API、Schema、菜单、状态或领域模型的正式实现任务。
3. 按真实依赖切分；没有依赖的单元可并行，有依赖的写清输入输出。
4. 每个单元只设一个主要结果和一个可验证终点。
5. 标出共享写入面、外部副作用、未知事实与需要用户决策的事项。
6. 根据执行中的新证据更新计划，不把最初拆解当不可变合同。
