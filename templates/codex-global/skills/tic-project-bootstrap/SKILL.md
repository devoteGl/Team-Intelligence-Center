...
2. Otherwise use fallback rules source: `{{TIC_RULES_DIR}}`.
3. Read and follow `<rules_dir>/Skills/project-governance-bootstrap.md` when governance setup is requested.
4. For lightweight install, prefer `<rules_dir>/tools/install.sh` or `<rules_dir>/tools/install.ps1`.
5. Do not copy TIC Skills into project-local or global skills by default.
